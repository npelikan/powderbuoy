...
hist_data <- 2008:2018 %>%
    map_dfr(function(year){
        res <- GET(
            glue("https://www.ndbc.noaa.gov/view_text_file.php?filename=51101h{year}.txt.gz&dir=data/historical/stdmet/")
        )
...
jan_sept_19_data <- list(mnth = month.abb[1:9], mc = 1:9) %>%
    pmap_dfr(function(mnth, mc){
        "https://www.ndbc.noaa.gov/data/stdmet/{mnth}/51101{mc}2019.txt.gz" %>%
            glue() %>%
...
oct19_data <- "https://www.ndbc.noaa.gov/view_text_file.php?filename=51101a2019.txt.gz&dir=data/stdmet/Oct/" %>%
    GET() %>% content() %>% parse_buoy_data()

nov19_data <- "https://www.ndbc.noaa.gov/data/stdmet/Nov/51101.txt" %>%
    GET() %>% content() %>% parse_buoy_data()

current_data <- "https://www.ndbc.noaa.gov/data/realtime2/51101.txt" %>%
    GET() %>% content() %>% parse_buoy_data(na = c("", "NA", "MM"))
