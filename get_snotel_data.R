...
raw_df <- "https://wcc.sc.egov.usda.gov/reportGenerator/view_csv/customSingleStationReport/daily/366:UT:SNTL|id=%22%22|name/POR_BEGIN,POR_END/WTEQ::value,PREC::value,TMAX::value,TMIN::value,TAVG::value,PRCP::value" %>%
    GET() %>% content() %>%
...
clean_df <- raw_df %>%
    ...
    write_csv("data/366_snotel_data.csv.gz")
